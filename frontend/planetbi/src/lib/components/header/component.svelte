<header>
  <a class="logo" href="/">
    <!-- <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      class="s-pDkRMPybvvqn"
      ><circle cx="12" cy="12" r="9" class="s-pDkRMPybvvqn"></circle><path
        stroke-linecap="round"
        d="M15.9,3.75a9.81,9.81,0,0,1,.85-.5,16.88,16.88,0,0,1,1.58-.75,8.34,8.34,0,0,1,1.36-.4,3.1,3.1,0,0,1,1.09,0,1.43,1.43,0,0,1,.77.37,1.43,1.43,0,0,1,.37.77,3.1,3.1,0,0,1,0,1.09,8.34,8.34,0,0,1-.4,1.36,16.88,16.88,0,0,1-.75,1.58c-.3.56-.66,1.15-1.07,1.76s-.86,1.24-1.36,1.88-1,1.3-1.63,2-1.2,1.32-1.86,2-1.32,1.28-2,1.86-1.32,1.13-2,1.63-1.27,1-1.88,1.36-1.2.77-1.76,1.07a16.88,16.88,0,0,1-1.58.75,8.34,8.34,0,0,1-1.36.4,3.1,3.1,0,0,1-1.09,0,1.43,1.43,0,0,1-.77-.37,1.43,1.43,0,0,1-.37-.77,3.1,3.1,0,0,1,0-1.09,8.34,8.34,0,0,1,.4-1.36,16.88,16.88,0,0,1,.75-1.58c.3-.56.5-.9.5-.9"
        class="s-pDkRMPybvvqn"
      ></path></svg
    >
    <span>Planet<span class="bi">BI</span></span> -->

    <svg
      xmlns="http://www.w3.org/2000/svg"
      xmlns:xlink="http://www.w3.org/1999/xlink"
      viewBox="0 0 307.8 315"
      ><path
        d="M745,220q17.89,78.4,35.8,156.8L649,265l91,270L515,424l119-49L473,220Z"
        transform="translate(-473 -220)"
      ></path></svg
    >
    <!-- <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 437 437">
      <path
        d="m148.91,437c20.73-35.19,47.51-64.26,84.13-92.93.33-.28.69-.59,1.22-.95,11.96-8.33-.55-14.85-11.63-22.72-16.23-11.49-30.29-29.9-26.99-32.72,4.07-3.47,11.55,17.44,42.76,29.07,17.78,6.63,21.97,3.55,43.11-6.45.14-.05.27-.09.38-.15,6.24-3.76,12.53-7.39,18.96-10.73,34.24-17.88,62.58-44.13,73.71-64.14l.02.02c37.5-63.15,20.56-131.79-11.94-171.29-3.11-3.78-7.77-8.07-10.39-10.65-4.02,4.54-8.94,12.45-6.31,21.8,4.37,15.63,2.96,30.52-5.42,40.14,8.93,27.28-.31,58.67-21.06,76.63-7.07,4.55-14.27,8.13-22.52,10.1,0,0-6.72,1.57-13.77,1.68-23.21.35-68.76-28.25-83.15-36.31,0,0-38.17-21.37-87-21-17.05.13-62.22,3.7-113.02,49.34V54.68C0,24.48,24.43,0,54.68,0h327.63c30.2,0,54.68,24.43,54.68,54.68v327.63c0,30.2-24.43,54.68-54.68,54.68-77.8,0-155.6,0-233.4,0Zm3.85-214.13c-4.29,6.84-13.3,8.89-20.14,4.6-6.82-4.28-8.9-13.3-4.61-20.13,4.3-6.84,13.32-8.9,20.15-4.62,6.84,4.3,8.9,13.31,4.61,20.15h0Zm145.68-129.8c13.55,38.78,46.05,21.82,38.99-15.1-3.81-19.93,6.01-30.8,6.01-30.8-33.78-13-54.88,17.6-45,45.89Zm-189.12,167.33s14.62,32.43,3.56,75.9c-4.06,15.83-22.83,30.15-53.21,30.19-6.36-5.73-12.32-12.03-17.8-18.79,24.64,6.2,49.24,1.04,58.14-9.81,14.29-17.4,9.31-77.49,9.31-77.49Z"
      ></path>
    </svg> -->
    <span>Plainbyte</span>
  </a>
  <div class="search">
    <div class="placeholder">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 16 16"
        fill="currentColor"
        class="w-4 h-4"
      >
        <path
          fill-rule="evenodd"
          d="M9.965 11.026a5 5 0 1 1 1.06-1.06l2.755 2.754a.75.75 0 1 1-1.06 1.06l-2.755-2.754ZM10.5 7a3.5 3.5 0 1 1-7 0 3.5 3.5 0 0 1 7 0Z"
          clip-rule="evenodd"
        ></path>
      </svg>
      Search
    </div>
  </div>
  <div class="nav">
    <!-- <div onclick="dropdown(this)" class="item create dropbtn">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="dropbtn">
            <path class="dropbtn" stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
          </svg>
          <div class="dropdown">
            <a href="#" onclick="showCreateDashboardModal()" class="item">
              Dashboard<div class="desc">Where you monitor data changes</div></a>
            <a href="#" onclick="showCreateMonitorModal()" class="viewset">
              Viewset<div class="desc">Collection of views</div></a>
            <a href="#" onclick="showCreateViewModal()" class="view">
              View<div class="desc">Single aspect of your data</div></a>
          </div>
        </div>
        <div class="separator"></div> -->
    <a class="home item active" href="/home">
      <svg xmlns="http://www.w3.org/2000/svg" version="1.2" viewBox="0 0 24 24">
        <path
          d="M12 3s-6.186 5.34-9.643 8.232A1.041 1.041 0 0 0 2 12a1 1 0 0 0 1 1h2v7a1 1 0 0 0 1 1h3a1 1 0 0 0 1-1v-4h4v4a1 1 0 0 0 1 1h3a1 1 0 0 0 1-1v-7h2a1 1 0 0 0 1-1 .98.98 0 0 0-.383-.768C18.184 8.34 12 3 12 3z"
        ></path>
      </svg>
      Home
    </a>
    <a class="store item" href="/store">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
        <path
          fill-rule="evenodd"
          d="M6 5v1H4.667a1.75 1.75 0 00-1.743 1.598l-.826 9.5A1.75 1.75 0 003.84 19H16.16a1.75 1.75 0 001.743-1.902l-.826-9.5A1.75 1.75 0 0015.333 6H14V5a4 4 0 00-8 0zm4-2.5A2.5 2.5 0 007.5 5v1h5V5A2.5 2.5 0 0010 2.5zM7.5 10a2.5 2.5 0 005 0V8.75a.75.75 0 011.5 0V10a4 4 0 01-8 0V8.75a.75.75 0 011.5 0V10z"
          clip-rule="evenodd"
        ></path>
      </svg>
      Store
    </a>

    <div onclick="dropdown(this)" class="menu item dropbtn">
      <svg
        class="dropbtn"
        viewBox="0 0 290 290"
        style="enable-background: new 0 0 290 290"
        xml:space="preserve"
      >
        <g class="dropbtn">
          <path
            class="dropbtn"
            d="M255,110c-19.299,0-35,15.701-35,35s15.701,35,35,35s35-15.701,35-35S274.299,110,255,110z"
          ></path>
          <path
            class="dropbtn"
            d="M35,110c-19.299,0-35,15.701-35,35s15.701,35,35,35s35-15.701,35-35S54.299,110,35,110z"
          ></path>
          <path
            class="dropbtn"
            d="M145,110c-19.299,0-35,15.701-35,35s15.701,35,35,35s35-15.701,35-35S164.299,110,145,110z"
          ></path>
        </g>
      </svg>
      <div class="dropdown nav-dropdown">
        <a href="/datasources" class="datasources">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="none"
          >
            <ellipse cx="12" cy="7" rx="7" ry="3"></ellipse>
            <path
              d="M5 13C5 13 5 15.3431 5 17C5 18.6569 8.13401 20 12 20C15.866 20 19 18.6569 19 17C19 16.173 19 13 19 13"
              stroke-linecap="square"
            ></path>
            <path
              d="M5 7C5 7 5 10.3431 5 12C5 13.6569 8.13401 15 12 15C15.866 15 19 13.6569 19 12C19 11.173 19 7 19 7"
            ></path>
          </svg>
          Datasources
        </a>
        <a href="/runners" class="runners">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 36 36"
            version="1.1"
            preserveAspectRatio="xMidYMid meet"
          >
            <path
              d="M23.08,23.07h-11v1.5H23.83a.75.75,0,0,0,.75-.75V11.33h-1.5Z"
              class="clr-i-outline clr-i-outline-path-1"
            ></path>
            <path
              d="M32.2,18.15a.8.8,0,1,0,0-1.6H30v-5.4h2.2a.8.8,0,1,0,0-1.6H30V8.1A2.1,2.1,0,0,0,27.9,6H26.35V3.8a.8.8,0,1,0-1.6,0V6h-5.4V3.8a.8.8,0,1,0-1.6,0V6h-5.4V3.8a.8.8,0,1,0-1.6,0V6H8.1A2.1,2.1,0,0,0,6,8.1V9.55H3.8a.8.8,0,1,0,0,1.6H6v5.4H3.8a.8.8,0,1,0,0,1.6H6v5.4H3.8a.8.8,0,1,0,0,1.6H6V27.9A2.1,2.1,0,0,0,8.1,30h2.65v2.2a.8.8,0,1,0,1.6,0V30h5.4v2.2a.8.8,0,1,0,1.6,0V30h5.4v2.2a.8.8,0,1,0,1.6,0V30H27.9A2.1,2.1,0,0,0,30,27.9V25.15h2.2a.8.8,0,1,0,0-1.6H30v-5.4ZM28,27.9a.1.1,0,0,1-.1.1H8.1a.1.1,0,0,1-.1-.1V8.1A.1.1,0,0,1,8.1,8H27.9a.1.1,0,0,1,.1.1Z"
              class="clr-i-outline clr-i-outline-path-2"
            ></path>
            <rect x="0" y="0" width="36" height="36" fill-opacity="0"></rect>
          </svg>
          Runners
        </a>
        <a href="/account" class="account">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            stroke-width="1.5"
            stroke="currentColor"
            class="w-6 h-6"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              d="M17.982 18.725A7.488 7.488 0 0012 15.75a7.488 7.488 0 00-5.982 2.975m11.963 0a9 9 0 10-11.963 0m11.963 0A8.966 8.966 0 0112 21a8.966 8.966 0 01-5.982-2.275M15 9.75a3 3 0 11-6 0 3 3 0 016 0z"
            ></path>
          </svg>
          Account
        </a>
        <a href="#support" class="support">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            stroke-width="1.5"
            stroke="currentColor"
            class="w-6 h-6"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z"
            ></path>
          </svg>
          Support
        </a>
        <a href="/dev/docs" target="_blank" class="documentation">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="none"
          >
            <path
              d="M3 10C3 6.22876 3 4.34315 4.17157 3.17157C5.34315 2 7.22876 2 11 2H13C16.7712 2 18.6569 2 19.8284 3.17157C21 4.34315 21 6.22876 21 10V14C21 17.7712 21 19.6569 19.8284 20.8284C18.6569 22 16.7712 22 13 22H11C7.22876 22 5.34315 22 4.17157 20.8284C3 19.6569 3 17.7712 3 14V10Z"
              stroke-width="1.5"
            ></path>
            <path d="M8 12H16" stroke-width="1.5" stroke-linecap="round"></path>
            <path d="M8 8H16" stroke-width="1.5" stroke-linecap="round"></path>
            <path d="M8 16H13" stroke-width="1.5" stroke-linecap="round"></path>
          </svg>
          Documentation
        </a>
        <a onclick="signout()" class="signout">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="none"
          >
            <path
              d="M8.90002 7.55999C9.21002 3.95999 11.06 2.48999 15.11 2.48999H15.24C19.71 2.48999 21.5 4.27999 21.5 8.74999V15.27C21.5 19.74 19.71 21.53 15.24 21.53H15.11C11.09 21.53 9.24002 20.08 8.91002 16.54"
              stroke-width="1.5"
              stroke-linecap="round"
              stroke-linejoin="round"
            ></path>
            <path
              d="M15 12H3.62"
              stroke-width="1.5"
              stroke-linecap="round"
              stroke-linejoin="round"
            ></path>
            <path
              d="M5.85 8.6499L2.5 11.9999L5.85 15.3499"
              stroke-width="1.5"
              stroke-linecap="round"
              stroke-linejoin="round"
            ></path>
          </svg>
          Sign out
        </a>
      </div>
    </div>
  </div>
</header>

<style>
  @import "component.css";
</style>
